...
<body>
    ...
    <div class="packages-section">
        ...
        <?php
        if ($destination) {
            $sql = "SELECT * FROM itinerary_activities WHERE destination LIKE ?";
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    // echo "<h2 class='day'>Day:" . htmlspecialchars($row['day']) . "</h2>";
                    // echo "<div class='package'>";
                    // echo "<img src='" . htmlspecialchars($row['image_url']) . "' alt='" . htmlspecialchars($row['destination']) . "'>";
                    // echo "<div class='info'>";
                    // echo "<h4 class='package-name'>Day: " . htmlspecialchars($row['day']) . "</h4>";
                    // echo "<p class='package-price'>" . htmlspecialchars($row['activity']) . "</p>";
                    // echo "</div>";
                    echo "<div class='package'>";
                    echo "<img src='" . htmlspecialchars($row['image_url']) . "' alt='" . htmlspecialchars($row['destination']) . "'>";
                    // echo "<p class='package-name'>Day: " . htmlspecialchars($row['day']) . "</p>";
                    echo "<p class='package-name'> Day " . htmlspecialchars($row['day']) . " , ". htmlspecialchars($row['time_of_day']) . " : " . htmlspecialchars($row['activity']) . "</p>";
                    // echo "<p class='package-name'> " . htmlspecialchars($row['time_of_day']) . "</p>";
                    echo "<div class='quick-view'>";
                    echo "<a href='./edit_activity.php?id=" . htmlspecialchars($row['id']) . "&destination=" . urlencode($destination) . "' class='quick-view' style='background-color: blue;'>Edit</a>";
                    echo "<a href='./delete_activity.php?id=" . htmlspecialchars($row['id']) . "&destination=" . urlencode($destination) . "' class='quick-view' style='background-color: #A888B5;'>Delete</a>";
